...
describe("GMT Token", function () {
    ...
    beforeEach(async () => {
        ...
        GMTToken = await ethers.getContractFactory("GMTToken");
    ...
    it("should revert if mint more than max supply", async function(){
        await gmtToken.connect(owner).mint("6000000000000000000000000000000")
        let totalSupply = await gmtToken.totalSupply()
        totalSupply = new BigNumber(totalSupply.toString())
        let _totalSupply = BigNumber("6000000000000000000000000000")
        expect(totalSupply.toString()).to.equal(_totalSupply.toString());

        //mint more than max supply
        // await expect(gmtToken.connect(owner).mint("10000000000000000000000000000")).to.be.revertedWith("GMT Token: total supply exceeded")
