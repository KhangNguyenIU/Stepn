...
describe("Sneaker NFT", function () {
    ...
    describe("Upgrade sneaker", function () {
        ...
        it("should leveling sneaker successful", async () => {
            ...
            // console.log({levelAfter})
            expect(levelAfter.toString()).to.equal(String(Number(settings.update.leveling.levelPerUpdate)))
        ...
        it("should unlock reward GMT at level 30", async ()=>{
            ...
            await Promise.all(Array(6).fill(0).map(async ()=>{
                await (await sneakerInstance.connect(user1).levelingSneaker(settings.newSneaker.one.id)).wait();
            }))
